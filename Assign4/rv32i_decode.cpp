...
string rv32i_decode::render_csrrx(uint32_t insn, const char *mnemonic)
{
	...
	os << render_mnemonic(mnemonic) << render_reg(get_rd(insn)) << "," << "0x" << setfill('0') << right << setw(3) << std::hex << get_imm_i(insn) << "," << render_reg(get_rs1(insn));
...
string rv32i_decode::render_csrrxi(uint32_t insn, const char *mnemonic)
{
	...
	os << render_mnemonic(mnemonic) << render_reg(get_rd(insn)) << "," << "0x" << setfill('0') << right << setw(3) << std::hex << get_imm_i(insn) << "," << std::dec << get_rs1(insn);
